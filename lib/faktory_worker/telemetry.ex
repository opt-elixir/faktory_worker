defmodule FaktoryWorker.Telemetry do
  ...
  defp log_event(:ack, %{status: :error}, job) do
    ...
    level =
      case job.reason do
        # Generated by `FaktoryWorker.Worker.Server.terminate/2` when
        # the OTP behaviour invokes `terminate` as it's being shutdown.
        # Worker termination happens during routine shutdown and likely
        # doesn't indicate a real failure. So long as the job has retries
        # configured, it will be picked back up by the next available worker.
        "Worker Terminated" -> :warn
        _ -> :error
      end
